...
const NavBar = () => {
    ...
    const [windowSize, setWindowSize] = useState<{ innerWidth: number; innerHeight: number } | null>(getWindowSize());
    ...
    useEffect(() => {
        if (windowSize && windowSize?.innerWidth < 1000) {
            setIsMobile(true);
        } else {
            setIsMobile(false);
        }
        ...
        <nav className={styles.wrapper}>
            ...
            <AnimatePresence>
                {isMobile && menuVisible && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={styles.mobileMenuWrapper}>
                        <motion.div
                            initial={{ x: "1000px" }}
                            animate={{ x: "0px" }}
                            exit={{ x: "1000px" }}
            ...
            {isMobile && !menuVisible && (
                <button className={styles.hamburger} onClick={() => setMenuVisible(true)}>
